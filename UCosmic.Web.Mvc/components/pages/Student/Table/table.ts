...
//},
    ...
    , tenant_id_changed: function (nsad) {
        ...
        //if(!this.mobility_snapshot.length){
        //}
        worker.postMessage('go');//, list: list);
        worker.addEventListener('message', (e: any) => {
            var call_start_setup_filter = (value) => {
                this.mobilities = _.uniq(_.union(JSON.parse(value), this.mobilities), 'key');
                this.start_setup_filter()
            }
            if (e.data.type == 'mobility_list') {
                call_start_setup_filter(e.data.value);
            } else if (e.data.type == 'term_list') {
                this.term_list = JSON.parse(e.data.value);
            } else if (e.data.type == 'establishment_list') {
                this.establishment_list = JSON.parse(e.data.value);
            } else if (e.data.type == 'country_list') {
                this.country_list = JSON.parse(e.data.value);
            } else if (e.data.type == 'level_list') {
                this.level_list = JSON.parse(e.data.value);
            } else if (e.data.type == 'program_list') {
                this.program_list = JSON.parse(e.data.value);
            } 
            //switch (e.data.type) {
            //    case "term_list":
            //        this.term_list = JSON.parse(e.data.value);
            ...
            //    //default: return "";
            //}
            //this.mobilities = JSON.parse(e.data)
            //temp_snapshot.push(JSON.parse(e.data))
            //if (all_loaded) {
            //    _.forEach(temp_snapshot, (value) => {
            //        this.mobilities.push(
            //            {
            //                country_name: _.result(_.find(this.country_list, { '_id': value.country }), 'text'), //
            //                level_name: _.result(_.find(this.level_list, { '_id': value.level }), 'text'), //this.level_list[value.level] ? this.level_list[value.level].text : "",
            //                program_name: _.result(_.find(this.program_list, { '_id': value.program }), 'text'), //this.program_list[value.program] ? this.program_list[value.program].text : "",
            //                student_name: _.result(_.find(this.student_list, { '_id': value.student }), 'text'), //this.student_list[value.student] ? this.student_list[value.student].text : "",
            //                term_name: _.result(_.find(this.term_list, { '_id': value.term }), 'text'), //this.term_list[value.term] ? this.term_list[value.term].text : "",
            //                status: value.status == 'IN' ? "In Coming" : "Out Going",
            //                affiliation_name: this.establishment_list[value.affiliation] ? this.establishment_list[value.affiliation].text : "",
            //                sub_affiliation_name: this.establishment_list[value.sub_affiliation] ? this.establishment_list[value.sub_affiliation].text : "",
            //                foreign_affiliation_name: this.establishment_list[value.foreign_affiliation] ? this.establishment_list[value.foreign_affiliation].text : "",
            //            }
            //            );
            //    })
            //    temp_snapshot = [];
