Polymer({is:'is-page-summary-report', {
    isAjaxing: false,
    activityTypeCounts: [],
    agreementTypeCounts: [],
    establishmentSearch: "",
    ...
    filter: function (ctx, next) {
        ...
        if (establishment_id != myThis.last_selected_establishment_id || place_id != myThis.last_selected_place_id || tenant_id != myThis.last_tenant_id) {
            ...
            if (!myThis.affiliations) {
                myThis.$.ajax_affiliations.generateRequest();
            }
            else {
            ...
            if (!myThis.selectedEstablishmentId) {
                myThis.$.ajax_activities.generateRequest();
                myThis.activityTypeCountsLoaded = false;
            }
            myThis.$.ajax_agreements.generateRequest();
            myThis.$.ajax_degrees.generateRequest();
    ...
    establishmentListSearch: function (event, detail, sender) {
        ...
        if (this.establishmentSearch == "") {
            ...
            if (this.isAjaxing != true) {
                this.isAjaxing = true;
                this.$.ajax_establishmentsList.generateRequest();
                ...
                setTimeout(function () {
                    if (_this.lastEstablishmentSearch != _this.establishmentSearch) {
                        ...
                        _this.$.ajax_establishmentsList.generateRequest();
    ...
    new_tenant_idChanged: function (newValue, oldValue) {
