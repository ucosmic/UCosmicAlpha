...
self.menu_drop_down_selected = function (_id) {
    ...
    if (url == 'restaurant') {
        window.location = '/index2.html#!/ucosmic/my_restaurant';
    } else {
                    ...
                    ucosmic.fire_ref_user.on("value", function (profile) {
                        ...
                        if (ucosmic.user && ucosmic.user.email && profile.val()) {
                            ...
                            ucosmic.user = {
                                ...
                                uid: authData.uid,
                                restaurant_name: user.restaurant_name ? user.restaurant_name : profile.val() && profile.val().restaurant_name ? profile.val().restaurant_name : 'Not set'
                            };
                            if (!profile.val() && user.email) {
                                ucosmic.fire_ref.child("Emails").push({
                                    ...
                                    html: user.first_name + ", <br/><br/>Thank you for registering for " + restaurant_name + ". \
                                    <br/><br/> Please var us know if you have any questions! <br /> <br /> \
                                    Sincerely <br /> " + restaurant_name + " Staff",
                                    text: user.first_name + ", thank you for registering for " + restaurant_name + ". \
                                    Please var us know if you have any questions! \
                                    Sincerely - " + restaurant_name + " Staff",
                                    is_sent: false,
                        ...
                        self.pages.push({ title: 'Restaurant' });
                    });
                } else {
                        if (ucosmic.user) {
                            ucosmic.user = {};
