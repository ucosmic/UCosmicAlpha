riot.tag2('textbox', '<div> <label id="label" for="input" class="pre_scale_bottom_center {fade_in: is_shown} {fade_out: !is_shown} {slide_top_bottom: !is_shown}">{opts.place_holder}</label> <br> <input type="{opts.type}" pattern="{opts.pattern}" placeholder="{opts.place_holder}" name="{opts.name}" onkeyup="{show_label}" id="input" autocomplete="on" maxlength="{opts.max_length}" __disabled="{opts.disabled}" value="{opts.input_value}" riot-style="width: {opts.width}; height: {opts.height}"> <br> <span class="validation_message" show="{validation_message}" riot-style="width: {opts.width}; height: {opts.height}"><span>{validation_message}</span></span> </div>', 'textbox label{ padding: 4px; } textbox input{ border-width: 0 0 1px 0; border-color: #948174; padding: 4px; margin-bottom:10px; } textbox input:hover { background: #FFFFC1; border-width: 0 0 1px 0; border-color: #948174; } textbox input:focus { background: #FFFF88; border-width: 0 0 1px 0; border-color: #948174; outline: 0; } textbox input:focus:hover { background: #FFFF88; border-width: 0 0 1px 0; border-color: #948174; outline: 0; } textbox .validation_message{ position: absolute; height: 0; overflow-y:visible; color: red; } textbox .validation_message span{ position: relative; top: -10px; }', '', function(opts) {
"use strict";
var self = this;
self.is_shown = false;
