...
self.on('mount', function () {
    ...
    var fire_ref_countries = new Firebase("https://UCosmic.firebaseio.com/Places/Countries");
    fire_ref_countries.on("value", function (snapshot) {
        self.countries = snapshot.val();
    });

    var data_list_stream = Kefir.sequentially(0, [{ url: "https://UCosmic.firebaseio.com/Members/" + self.opts.tenant_id + "/Mobilities/Counts/OUT", direction: 'out' }, { url: "https://UCosmic.firebaseio.com/Members/" + self.opts.tenant_id + "/Mobilities/Counts/IN", direction: 'in' }]);
    ...
    fire_ref_est_ances.on("value", function (snapshot) {
        ...
        Object.keys(sub_affiliations).forEach(function (key) {
            return self.sub_affiliations_ancestors[parseInt(key)] = sub_affiliations[key];
        });
        sub_affiliations_level_1_get();
    });
});
