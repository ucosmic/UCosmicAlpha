<auto_drop_down>
    ...
    <div class="layout vertical">
        <div id="title">
                <div class="layout horizontal end">
                    <div onclick="{toggle_open}">
                        <div>
                            <div class="{float_text: selected_item} " show="{selected_item}"><div>{opts.title}</div></div>
                            <textbox id="input1" onKeyUp="{filter}" input_autocomplete="off" input_style="background-color: transparent;
                                border: none;
                                padding: 0;
                                line-height: 20px;
                                height: 25px;"   no_clear="true" show="{!selected_item}" id="text" type="text" name="text" place_holder="{opts.title}" width="170px;" autofocus></textbox>
                            <textbox id="input2" onKeyUp="{filter}" input_autocomplete="off" input_style="background-color: transparent;
                                border: none;
                                padding: 0;
                                line-height: 20px;
                                height: 25px;"  no_clear="true" show="{selected_item}" id="text" type="text" name="text" place_holder="{opts.title}"  width="170px;" input_value="{selected_item.title}" autofocus></textbox>
                            <!--<div show="{!selected_item}"><div>Select {opts.title}</div></div>-->
                            <!--<div style="font-weight: bold" show="{selected_item}">{selected_item.title}<span if="{selected_item.cost}" data-_id="{i}">&nbsp;{ttw.currency(selected_item.cost)}</span></div>-->
                        </div>
                    </div>
                    <div class="flex"></div>
                    <div style="height:20px; width:40px; cursor: pointer;" onclick="{toggle}">
                        <svg viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet" class="style-scope iron-icon" style="pointer-events: none; display: block; width: 100%; height: 100%;">
                            <g class="style-scope iron-icon">
                                <path show="{is_shown}" d="M12 8l-6 6 1.41 1.41L12 10.83l4.59 4.58L18 14z" class="style-scope iron-icon">
                                </path>
                                <path show="{!is_shown}" d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6z" class="style-scope iron-icon">
                                </path>
                            </g>
                        </svg>
                    </div>
        ...
        <div id="ddl_container" riot-style=" direction: {opts.direction}; left: {opts.ddl_container_left}; overflow:{is_shown ? 'visible': 'hidden'};">
            <div id="list_div" class="{opts.pre_scale_class} {fade_in: is_shown} {fade_out: !is_shown} {scale: !is_shown && opts.scale_type != 'scale_height'}
            {scale_height: !is_shown && opts.scale_type == 'scale_height'}" >
                <ul id="list_ul" riot-style="background-color:{opts.background_color};  max-height:{opts.max_height}; width:{opts.max_width} ">
                    ...
                    <li class="layout horizontal start-justified highlight-text" each="{ item, i in list }" onclick="{select_item}" data-_id="{item._id}">
                        <span style="display: flex; white-space: normal;" data-_id="{item._id}" data-text="{item.title}">{item.title}</span>
                    </li>
                    <li show="{!list}" class="layout horizontal start-justified">
                        <span  style="display: flex; white-space: normal;">Loading</span>
                        <is_loading is_showing="true" show="true" height="20px" width="20px;" container_width="80px;"  color="green">
                        </is_loading>
                    </li>
                </ul>
            </div>
    ...
    <script type="es6">
        ...
        self.select_item = function(event, id){
            ...
            // self.input1._tag.is_shown ? self.input1._tag.input.value = event.target.dataset.text : self.input1._tag.input.value = "";
            // self.input2._tag.is_shown ? self.input2._tag.input.value = event.target.dataset.text : self.input2._tag.input.value = "";
            if(id !== undefined){
        ...
        self.filter = (e, text) => {
            ...
            if (text == "") {
                ...
                self.list = _.filter(self.original_list, (item) => {
        ...
        self.find_selected_item = (default_item) => {
            self.selected_item = self.opts.selected_item_id != undefined && self.list ? self.list.find( (val, index) => { return val._id == self.opts.selected_item_id}) : default_item && default_item.length > 0 ? default_item[0] : null;
        ...
        self.update_original_list = (list) => {
        ...
        self.find_function = (func_or_string) => {
