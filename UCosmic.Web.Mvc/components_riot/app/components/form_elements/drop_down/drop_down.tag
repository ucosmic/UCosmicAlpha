<drop_down>
    ...
    <div class="layout vertical">
        <div id="title">
            <div class="layout horizontal end" onclick="{toggle}">
                <div show="{!opts.title}">
                    <yield/>
                </div>
                <div show="{opts.title}">
        ...
        <div id="ddl_container" riot-style=" direction: {opts.direction}; left: {opts.ddl_container_left}; overflow:{is_shown ? 'visible': 'hidden'};">
            ...
            {scale_height: !is_shown && opts.scale_type == 'scale_height'}" >
                <ul id="list_ul" riot-style="background-color:{opts.background_color};  max-height:{opts.max_height}; width:{opts.max_width} ">
                    ...
                    <li class="layout horizontal start-justified highlight-text" each="{ item, i in list }" onclick="{select_item}" data-_id="{i}">
                        <span style="display: flex; white-space: normal;" data-_id="{i}">{item.title}</span><span if="{item.cost}" data-_id="{i}">&nbsp;{ttw.currency(item.cost)}</span>
                    </li>
                </ul>
            </div>
        </div>
